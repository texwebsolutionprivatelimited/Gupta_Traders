-- Preserve receipt display values on POS sale lines.
...
begin
  ...
  for v in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.sale_items(sale_id, product_id, quantity, selling_price, discount, tax, total, product_name, sku, unit, unit_price, tax_rate, tax_amount, line_total, metadata)
    values(v_id, product_id, qty, price, disc, round((qty * price - disc) * rate / 100, 2), round(qty * price - disc, 2), item_name, case when product_id is null then null else p.sku end, item_unit, price, rate, round((qty * price - disc) * rate / 100, 2), round(qty * price - disc, 2),
      jsonb_build_object('display_price', coalesce((v->>'display_price')::numeric, price), 'item_discount_percent', coalesce((v->>'item_discount_percent')::numeric, 0), 'is_gst_inclusive', coalesce((v->>'is_gst_inclusive')::boolean, false)));
