...
import { FaShoppingCart as CartIcon, FaBalanceScale as ScaleIcon } from 'react-icons/fa'
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800/60">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/25">
            {items.length} items • {totalItems} qty
          </span>
        </div>
        <button
          onClick={onClearCart}
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-2">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500">
                  {formatINR(item.price)} × {item.quantity} {item.unit}
                  {item.isLoose && <ScaleIcon className="inline w-3.5 h-3.5 ml-1 text-violet-400" />}
                </p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-0 border border-slate-700/50 rounded-lg overflow-hidden">
                <button
                  onClick={() => onUpdateQuantity(item.cartId, Math.max(item.isLoose ? 0.1 : 1, item.quantity - (item.isLoose ? 0.25 : 1)))}
                  className="w-8 h-8 flex items-center justify-center bg-slate-800/80 text-slate-300 hover:bg-slate-700 hover:text-white transition-all text-lg font-bold"
                >
                  −
                </button>
                <input
                  type="number"
                  step={item.isLoose ? '0.001' : '1'}
                  min={item.isLoose ? '0.001' : '1'}
                  value={item.quantity}
                  onChange={(e) => {
                    const val = parseFloat(e.target.value)
                    if (val > 0) onUpdateQuantity(item.cartId, val)
                  }}
                ...
                />
                <button
                  onClick={() => onUpdateQuantity(item.cartId, item.quantity + (item.isLoose ? 0.25 : 1))}
                  className="w-8 h-8 flex items-center justify-center bg-slate-800/80 text-slate-300 hover:bg-emerald-600 hover:text-white transition-all text-lg font-bold"
                >
                  +
                </button>
              </div>
              ...
              <div className="flex items-center gap-1">
                {editingDiscount === item.cartId ? (
                  <input
                    ...
                    autoFocus
                    onBlur={(e) => {
                      onUpdateDiscount(item.cartId, parseFloat(e.target.value) || 0)
