...
export function subscribeToTable(table, onChange) { const channel=supabase.channel(`erp:${table}`).on('postgres_changes',{event:'*',schema:'public',table},onChange).subscribe(); return ()=>supabase.removeChannel(channel) }
