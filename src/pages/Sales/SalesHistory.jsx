...
function PaymentBadge({ payment }) {
  ...
  const currentStyle =
    styles[payment] || "bg-slate-500/10 text-slate-500 dark:text-slate-400";
...
export default function SalesHistory() {
  ...
  const filteredSales = useMemo(() => {
    ...
    return salesData.filter((sale) => {
      ...
      const invoice = String(sale?.invoice || "").toLowerCase();
  ...
  }, [salesData, search, statusFilter, paymentFilter]);
  ...
  const handleDownload = (sale) => {
    if (!sale) return;
    const invoiceNumber = sale.invoice || sale.id || "invoice";
    alert(`Downloading invoice ${invoiceNumber}`);
  };

  const handleInvoiceReprint = (sale) => {
    if (!sale) return;
    const invoice = sale.invoice || "";
    navigate(`/sales/invoice-reprint?invoice=${encodeURIComponent(invoice)}`);
  };
    ...
    <div className="min-h-screen bg-slate-50 px-4 py-6 text-slate-900 transition-colors dark:bg-slate-950 dark:text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Card title="Total Sales" value={filteredSales.length}/>
          <Card
            title="Sales Amount"
        ...
        {/* FILTER SECTION */}
        <div className="mb-6 rounded-2xl border border-slate-800/60 bg-slate-900/80 p-4 shadow-sm">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
            ...
            <select
              ...
              className="rounded-xl border border-slate-800/60 bg-slate-950/40 px-4 py-3 text-sm text-slate-200 outline-none transition focus:border-emerald-500/40 focus:ring-2 focus:ring-emerald-500/20 cursor-pointer"
            >
              <option value="All" className="bg-slate-900 text-slate-200">All Payments</option>
              ...
              <option value="Partial" className="bg-slate-900 text-slate-200">Partial</option>
            </select>
          </div>
        </div>
                ...
                {filteredSales.map((sale) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-center">
                      <PaymentBadge payment={sale.payment || "Pending"} />
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <button
                          ...
                          title="View Sale"
                          className="rounded-lg border border-slate-200 p-2 text-slate-500 transition hover:border-emerald-500 hover:bg-emerald-500/10 hover:text-emerald-500 dark:border-slate-700 dark:text-slate-400"
                        >
                          <EyeIcon />
                        </button>
                        ...
                        <button
                          type="button"
                          onClick={() => handleInvoiceReprint(sale)}
          ...
          >
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl bg-slate-50 p-4 dark:bg-slate-800/70">
                ...
                <div className="mt-2">
                  <PaymentBadge payment={selectedSale.payment || "Pending"} />
                </div>
              </div>
            </div>
